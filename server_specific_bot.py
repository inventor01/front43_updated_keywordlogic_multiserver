#!/usr/bin/env python3
"""
Front 43_updated_keywordlogic - Server-Specific Multi-Server Discord Bot
Enhanced bidirectional keyword matching with per-server configuration
Each server has its own unique keywords database and webhook configuration
"""
...
class MultiServerTokenBot(commands.Bot):
    def __init__(self):
    ...
    async def setup_hook(self):
        try:
            ...
            # Send startup notification to default webhook
            embed = {
                'title': '🤖 Front 43_updated_keywordlogic Multi-Server Bot ACTIVATED!',
                'description': f'6 Essential commands ready: add, remove, undo, list, clear, webhook',
                'color': 0x00ff41,
                'fields': [
                    {'name': '✅ Status', 'value': 'Multi-server bot connected and ready', 'inline': False},
                    {'name': '📊 Commands', 'value': 'Per-server keyword management enabled', 'inline': False},
                    {'name': '🔧 Setup', 'value': 'Use `/webhook` command to configure each server', 'inline': False}
                ]
            }
    ...
    async def on_ready(self):
        logger.info(f'Front 43_updated_keywordlogic Multi-Server Bot Active: {self.user}')
    ...
    def is_keyword_match(self, token_name: str, keyword: str) -> bool:
        """
        Enhanced bidirectional keyword matching algorithm
        Fixes the 'blue collar' vs 'blue collar boys' issue
        """
        try:
            # Normalize both inputs
            normalized_token = re.sub(r'[^\w\s]', '', token_name.lower().strip())
            normalized_keyword = re.sub(r'[^\w\s]', '', keyword.lower().strip())
            ...
            # Enhanced bidirectional word-based matching
            ...
            if keyword_words and token_words:
                ...
                intersection = token_words.intersection(keyword_words)
                
                if intersection:
                    # Method 1: Traditional - keyword words found in token (75% overlap)
                    overlap_ratio_1 = len(intersection) / len(keyword_words)
                    
                    # Method 2: Bidirectional - token words found in keyword (75% overlap)  
                    overlap_ratio_2 = len(intersection) / len(token_words)
            ...
            if len(keyword_words) == 1 and keyword_words.issubset(token_words):
                return True
...
async def add_keyword(interaction: discord.Interaction, keyword: str):
    ...
    try:
        ...
        embed = discord.Embed(
            title="✅ Keyword Added",
            description=f"Now monitoring: **{keyword}** in this server",
            color=0x00ff41,
...
async def remove_keyword(interaction: discord.Interaction, keyword: str):
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            description=f"Stopped monitoring: **{keyword}** in this server",
            color=0xff6b6b,
            timestamp=datetime.now()
        )
...
async def list_keywords(interaction: discord.Interaction):
    ...
    try:
        ...
        if not keywords:
            await interaction.followup.send(f"📝 No keywords found in **{interaction.guild.name}**. Use `/add <keyword>` to start monitoring.")
            return
        ...
        for chunk_idx, chunk in enumerate(chunks):
            embed = discord.Embed(
                title=f"📝 Your Keywords in {interaction.guild.name} {f'(Page {chunk_idx + 1})' if len(chunks) > 1 else ''}",
                description="\n".join(chunk),
                color=0x3498db,
                timestamp=datetime.now()
            )
            embed.add_field(name="📊 Total", value=f"{len(keywords)} keywords in this server", inline=True)
            embed.add_field(name="👤 User", value=f"<@{interaction.user.id}>", inline=True)
...
@bot.tree.command(name="clear", description="Remove ALL your keywords from this server (use with caution)")
async def clear_keywords(interaction: discord.Interaction, confirm: str):
    """Clear all keywords for the user in this server (requires confirmation)"""
    try:
        await interaction.response.defer()
        ...
        if confirm.lower() != "yes i want to delete all":
            embed = discord.Embed(
                ...
                description="To clear ALL your keywords in this server, use:\n`/clear confirm: yes i want to delete all`",
                color=0xffa500
            )
        ...
        cursor.execute("SELECT keyword FROM server_keywords WHERE server_id = %s AND user_id = %s", (server_id, user_id))
        keywords_to_delete = [row[0] for row in cursor.fetchall()]
        
        if not keywords_to_delete:
            await interaction.followup.send(f"📝 No keywords to clear in **{interaction.guild.name}**")
            cursor.close()
            conn.close()
            return
        ...
        # Delete all keywords for user in this server
        cursor.execute("DELETE FROM server_keywords WHERE server_id = %s AND user_id = %s", (server_id, user_id))
        conn.commit()
        ...
        embed = discord.Embed(
            ...
            description=f"Removed **{keyword_count}** keywords from monitoring in **{interaction.guild.name}**",
            color=0xff6b6b,
            timestamp=datetime.now()
        )
        embed.add_field(name="📊 Status", value="No keywords active in this server", inline=True)
...
@bot.tree.command(name="undo", description="Undo the last action in this server (add, remove, or clear)")
async def undo_keyword(interaction: discord.Interaction):
    """Undo the last action in this server (add, remove, or clear)"""
    try:
        await interaction.response.defer()
        ...
        if undo_key not in bot.undo_history:
            await interaction.followup.send(f"❌ No recent action to undo in **{interaction.guild.name}**")
            return
            
        last_action = bot.undo_history[undo_key]
        ...
        if action_type == 'add':
            ...
            embed = discord.Embed(
                ...
                description=f"Removed recently added keyword: **{keyword}** from **{interaction.guild.name}**",
                color=0xff6b6b,
                timestamp=datetime.now()
            )
            embed.add_field(name="📊 Remaining Keywords", value=f"{remaining_keywords}", inline=True)
            ...
            # Undo a remove action (add the keyword back)
            keyword = last_action['keyword']
            cursor.execute(
                "INSERT INTO server_keywords (server_id, user_id, keyword, created_at) VALUES (%s, %s, %s, %s) ON CONFLICT (server_id, user_id, keyword) DO NOTHING",
                (server_id, user_id, keyword, datetime.now())
            )
            conn.commit()
            ...
            embed = discord.Embed(
                title="↩️ Remove Action Undone",
                description=f"Restored keyword: **{keyword}** to **{interaction.guild.name}**",
                color=0x00ff41,
                timestamp=datetime.now()
            )
            embed.add_field(name="📊 Total Keywords", value=f"{total_keywords}", inline=True)
            ...
            embed = discord.Embed(
                ...
                description=f"Restored **{len(keywords_to_restore)}** keywords to **{interaction.guild.name}**",
                color=0x00ff41,
                timestamp=datetime.now()
            )
            embed.add_field(name="📊 Total Keywords", value=f"{total_keywords}", inline=True)
...
async def main():
    ...
    try:
        ...
        logger.info("🤖 Starting Front 43_updated_keywordlogic Multi-Server Discord Bot...")
